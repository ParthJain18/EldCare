...
const AddElder = () => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/addRelative', {
        patientEmail : patient
      })
      console.log(response)
      navigate('/relativedash')
      location.reload(); 
    }catch (err) {
      console.log(err)
    }
  ...
  return (
    ...
    <form className='mb-5' onSubmit={e => handleSubmit(e)}>   
      ...
      <div class="relative">
        ...
        <input type="email" id="default-search" class="block w-full p-4 pl-10 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 " placeholder="Add Elder By Email..." required onChange={(e) => setPatient(e.target.value)}/>
